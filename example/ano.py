...
def func(x):
    ...
    return np.array([total_cost*2, abs(h), length])
...
Z1, Z2, Z3 = func([X1, X2, X3])

# Flatten the Z1, Z2, and Z3 arrays
Z1_flat = Z1.ravel()
Z2_flat = Z2.ravel()
Z3_flat = Z3.ravel()

# Compute the RMS cost for each height
unique_heights = np.unique(Z2_flat)
rms_costs_height = []
for height in unique_heights:
    rms_costs_height.append(np.sqrt((Z1_flat[Z2_flat == height]**2).mean()))
rms_costs_height = np.array(rms_costs_height)

# Compute the RMS cost for each length of geogrid
unique_lengths = np.unique(Z3_flat)
rms_costs_length = []
for length in unique_lengths:
    rms_costs_length.append(np.sqrt((Z1_flat[Z3_flat == length]**2).mean()))
rms_costs_length = np.array(rms_costs_length)

# Compute the RMS cost for each Tu value
unique_tus = np.unique(X2.ravel())
rms_costs_tu = []
for tu in unique_tus:
    rms_costs_tu.append(np.sqrt((Z1_flat[(X2.ravel() == tu)]**2).mean()))
rms_costs_tu = np.array(rms_costs_tu)

# 1. Plot height (on y) vs RMS cost (on x)
plt.figure()
plt.scatter(rms_costs_height, unique_heights, marker='o', color='b', alpha=0.5)
plt.xlabel('RMS Total Cost (USD)')
plt.ylabel('Height of Retaining Wall (m)')
plt.title('RMS Cost vs Height')
plt.grid(True)
plt.show()

# 2. Plot Length of geogrid (on y) vs RMS cost (on x)
plt.figure()
plt.scatter(rms_costs_length, unique_lengths, marker='o', color='g', alpha=0.5)
plt.xlabel('RMS Total Cost (USD)')
plt.ylabel('Length of Geogrid (m)')
plt.title('RMS Cost vs Length of Geogrid')
plt.grid(True)
plt.show()

# 3. Plot RMS cost (on x) vs Tu (on y)
plt.figure()
plt.scatter(rms_costs_tu, unique_tus, marker='o', color='r', alpha=0.5)
plt.xlabel('RMS Total Cost (USD)')
plt.ylabel('Tu (kN/m)')
plt.title('RMS Cost vs Tu')
plt.grid(True)
plt.show()